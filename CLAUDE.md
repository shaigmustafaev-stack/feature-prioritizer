...
Приоритет провайдеров: `GROQ_API_KEY` → `OPENROUTER_API_KEY` → `GEMINI_API_KEY` → `ANTHROPIC_API_KEY`. Rate limit: 1 запрос / 30 сек на пользователя (in-memory Map). Timeout на клиенте: 30 сек через AbortController.
